...
static Font *FontNew( HBGL *pHBGL, const char *font_path )
{
...
static Font *SystemFontNew( HBGL *pHBGL, const char *font_name )
{
   char font_path[ 1024 ];
   FILE *fp = NULL;
...
HB_FUNC( SYSTEMFONTNEW )
{
   ...
   const char *font_name = hb_parc( 2 );

   if( pHBGL )
   {
      hb_retptr( SystemFontNew( pHBGL, font_name ) );
   }
   else
   {
      HB_ERR_ARGS();
...
/* FontNew( pHBGL, font_path ) --> pFont */
HB_FUNC( FONTNEW )
{
   HBGL *pHBGL = hb_parptr( 1 );
   const char *font_path = hb_parc( 2 );

   if( pHBGL )
   {
      hb_retptr( FontNew( pHBGL, font_path ) );
   }
   else
   {
      HB_ERR_ARGS();
